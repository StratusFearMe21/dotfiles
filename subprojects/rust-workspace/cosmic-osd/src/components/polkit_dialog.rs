...
use cosmic::iced_runtime::window::Id as SurfaceId;
use cosmic::{iced_core, theme, Renderer};
use iced::{
    wayland::{
        actions::layer_surface::SctkLayerSurfaceSettings,
        layer_surface::{KeyboardInteractivity, Layer},
    },
    widget,
    widget::svg,
...
pub struct State {
    ...
    echo: bool,
    text_input_id: iced_core::id::Id,
}

impl State {
    pub fn new<T: 'static>(id: SurfaceId, params: Params) -> (Self, Command<T>) {
        let text_input_id = iced_core::id::Id::unique();
    ...
    pub fn view(&self) -> Element<'_, Msg, Renderer> {
        ...
        let mut password_input = widget::text_input("", &self.password)
            .on_input(|password| Msg::Password(password))
