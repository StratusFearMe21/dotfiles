...
static mut SYSTEM_CONNECTION: MaybeUninit<zbus::Connection> = MaybeUninit::uninit();

async unsafe fn polkit_subscription() -> ReceiverStream<Event> {
    let (sender, receiver) = mpsc::channel(32);
    tokio::spawn(async move {
        // XXX unwrap
        register_agent(SYSTEM_CONNECTION.assume_init_ref(), sender)
            .await
            .unwrap();
        SYSTEM_CONNECTION.assume_init_drop();
    });
    ReceiverStream::new(receiver)
}

fn polkit_sub() -> impl Stream<Item = Event> {
    unsafe { polkit_subscription().flatten_stream() }
}

pub fn subscription(system_connection: zbus::Connection) -> iced::Subscription<Event> {
    unsafe {
        SYSTEM_CONNECTION.write(system_connection);
    }
    iced::subscription::run(polkit_sub)
}
