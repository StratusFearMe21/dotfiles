...
) -> Result<(std::time::Duration, Color), ParseError<'i, ()>> {
...
where
    F1: FnOnce(&mut DefaultColorParser<'a>, &mut Parser<'i, 't>) -> Result<R1, ParseError<'i, ()>>,
    F2: FnOnce(&mut DefaultColorParser<'a>, &mut Parser<'i, 't>) -> Result<R2, ParseError<'i, ()>>,
    F3: FnOnce(&mut DefaultColorParser<'a>, &mut Parser<'i, 't>) -> Result<R3, ParseError<'i, ()>>,
{
...
impl<'a, 'i> DefaultColorParser<'a> {
                ...
                match_ignore_ascii_case! { i.as_ref(),
                    "from" => {
                        ...
                        if let Some(ref mut cp) = self.reference() {
                            reference.convert_color(cp.as_str());
                        }
    ...
    fn parse_number<'t>(&mut self, input: &mut Parser<'i, 't>) -> Result<f32, ParseError<'i, ()>> {
        let location = input.current_source_location();
        Ok(match *input.next()? {
